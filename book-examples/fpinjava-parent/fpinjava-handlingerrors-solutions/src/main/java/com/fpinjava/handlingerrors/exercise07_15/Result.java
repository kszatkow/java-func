...
public abstract class Result<T> implements Serializable {
  ...
  public Result<T> filter(Function<T, Boolean> p) {
    return flatMap(x -> p.apply(x)
        ? this
        : failure("Brak dopasowania do warunku"));
  }
  ...
  private static class Empty<T> extends Result<T> {
    ...
    /**
     * Istnieje tylko jedna instancja Empty, więc wszystkie Empty są sobie równe.
     */
    @Override
  ...
  private static class Failure<T> extends Empty<T> {
    ...
    /**
     * Obiekty Failure są sobie równe, jeśli są tym samym obiektem.
     */
    @Override
    public boolean equals(Object o) {
      return this == o;
    ...
    /**
     * Potrzebujemy lepszej metody hashCode method zwracającej różne hashCode
     * dla wszystkich błędów. Patrz http://code.google.com/p/smhasher lub klasę
     * skrótu Guava jako przykład.
     */
    @Override
    public int hashCode() {
  ...
  public static <T> Result<T> of(Function<T, Boolean> predicate, T value) {
    try {
      ...
      String errMessage =
          String.format("Wyjątek w trakcie wyliczania predykatu: %s", value);
  ...
  public static <T> Result<T> of(Function<T, Boolean> predicate,
    ...
    try {
      ...
      String errMessage =
          String.format("Wyjątek w trakcie wyliczania predykatu: %s",
