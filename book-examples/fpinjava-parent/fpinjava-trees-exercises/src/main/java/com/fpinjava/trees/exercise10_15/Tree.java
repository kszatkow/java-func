...
public abstract class Tree<A extends Comparable<A>> {
  ...
  private static class T<A extends Comparable<A>> extends Tree<A> {
    ...
    public Tree<A> merge(A a, Tree<A> right) {
      return right.isEmpty()
          ? max().map(max -> max.compareTo(a) > 0 // Wartości domyślnej nigdy nie użyjemy, ale jest niezbędna.
          ? insert(a)
  ...
  private static <A extends Comparable<A>> boolean isUnBalanced(Tree<A> tree) {
    // Różnica musi wynosić 0, jeśli łączy rozmiar gałęzi jest parzysty i 1, jeśli jest nieparzysty.
