...
/**
 * Funkcyjna implementacja bazująca na fpinjava-common z kilkoma implementacjami main
 * wykorzystującymi read_ i readNumbers_ do zwracania wartości lub read i readNumbers
 * Functional Java implementation based upon fpinjava-common with several implementation of main
 * do obsługi NumberFormatException (i innych wyjątków) i zwracająca Result.
 */
public class Countdown0 {
  ...
  // Sprawdzanie wartości numerycznych.
  // ----------------------------------
  ...
  private static IO<Nothing> main0() {
    return Console.printLine("\nROZWIĄZYWANIE GRY W ODLICZANIE") // wyświetl pierwszy wiersz daych informacyjnych
                  ...
                  .map(Countdown0::readNumbers_) // zamień to na listę liczb całkowitych
                  .flatMap(ns -> Console.print("Wpisz liczbę docelową: ") // wyświetl drugie zapytanie
                                        ...
                                        .map(Countdown0::read_) // zamień ją na liczbę całkowitą
                                        .map(n -> solutions_(ns, n))) // oblicz wynik jako listę elementów Expr
  ...
  private static IO<Nothing> main0_() {
    return Console.printLine("\nROZWIĄZYWANIE GRY W ODLICZANIE") // wyświetl pierwszy wiersz daych informacyjnych
                  ...
                  .flatMap(x -> Console.print("Wpisz przekazane liczby: ")) // wyświetl pierwsze zapytanie
                  .flatMap(Console::readLine) // odczytaj listę liczb jako tekst
                  .map(Countdown0::readNumbers) // zamień na wynik będący listą liczb całkowitych
                  .flatMap(ns -> Console.print("Wpisz liczbę docelową: ") // wyświetl drugie zapytanie
                                        .flatMap(Console::readLine) // odczytaj liczbę w postaci tekstu
                                        .map(Countdown0::read) // zamień na wynik będący liczbą całkowitą
  ...
  private static IO<Nothing> main1() {
    ...
    IO<List<Integer>> numbers = Console.print("Wpisz przekazane liczby: ") // wyświetl pierwsze zapytanie
                                       .flatMap(Console::readLine) // odczytaj listę liczb jako tekst
                                       .map(Countdown0::readNumbers_); // zamień to na listę liczb całkowitych

    IO<Integer> number = Console.print("Wpisz liczbę docelową: ") // wyświetl drugie zapytanie
                                .flatMap(Console::readLine) // odczytaj liczbę w postaci tekstu
                                .map(Countdown0::read_); // zamień ją na liczbę całkowitą

    IO<List<Expr>> exprs = numbers.flatMap(ns -> number.map(n -> solutions_(ns, n))); // oblicz wynik jako listę elementów Expr
  ...
  public static void main2_() {
    System.out.println("\nROZWIĄZYWANIE GRY W ODLICZANIE");
    System.out.println("-----------------------------\n");
    Result<List<Integer>> ns = Console_.readLine("Wpisz przekazane liczby: ").flatMap(Countdown0::readNumbers);
    Result<Integer> n = Console_.readLine("Wpisz liczbę docelową: ").flatMap(Countdown0::read);
  ...
  public static Executable main4() {
    return () -> {
      System.out.println("\nROZWIĄZYWANIE GRY W ODLICZANIE");
      System.out.println("-----------------------------\n");
      Console_.readLine("Wpisz przekazane liczby: ").map(Countdown0::readNumbers_)
              .flatMap(ns -> Console_.readLine("Wpisz liczbę docelową: ").map(Countdown0::read_).map(n -> solutions_(ns, n)))
  ...
  public static Executable main4_() {
    return () -> {
      System.out.println("\nROZWIĄZYWANIE GRY W ODLICZANIE");
      System.out.println("-----------------------------\n");
      Console_.readLine("Wpisz przekazane liczby: ").flatMap(Countdown0::readNumbers)
              .flatMap(ns -> Console_.readLine("Wpisz liczbę docelową: ").flatMap(Countdown0::read).map(n -> solutions_(ns, n)))
              .forEachOrFail(System.out::println)
              .forEach(System.out::println);
    };
  }
  ...
  public static Executable main5() {
    return () -> {
      Console_.printLine("\nROZWIĄZYWANIE GRY W ODLICZANIE")
              ...
              .flatMap(x -> Console_.readLine("Wpisz przekazane liczby: ").map(Countdown0::readNumbers_)
  ...
  public static Executable main5_() {
    return () -> {
      Console_.printLine("\nROZWIĄZYWANIE GRY W ODLICZANIE")
              .flatMap(x -> Console_.printLine("-----------------------------\n"))
              .flatMap(x -> Console_.readLine("Wpisz przekazane liczby: ").flatMap(Countdown0::readNumbers)
                                    .flatMap(ns -> Console_.readLine("Wpisz liczbę docelową: ")
